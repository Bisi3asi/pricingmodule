...
extern "C" {
    double EXPORT_API pricing(
        ...
        , int couponDcb                             // INPUT 8.  쿠폰 지급 Day Count Basis       [30U/360 = 0, Act/Act = 1, Act/360 = 2, Act/365 = 3, 30E/360 = 4]
        , int businessDayConvention                 // INPUT 9.  영업일 계산방법                 [Following = 0, Modified Following = 1, Preceding = 2, Modified Preceding = 3]
        , int periodEndDateConvention               // INPUT 10.  기간 월말 계산방식             [Adjusted = 0, UnAdjusted = 1]
        , int accuralDcb                            // INPUT 11.  쿠폰 현재가 산출 일 계산방법   [30U/360 = 0, Act/Act = 1, Act/360 = 2, Act/365 = 3, 30E/360 = 4]
...
// , double* resultCsrDelta                         // OUTPUT 3. 결과값 ([index 0] array size, [index 1 ~ size] Csr Delta tenor, [size + 1 ~ end] Csr Delta Sensitivity)
        ...
        inputTradeInformation(notional, issueDateSerial, maturityDateSerial, revaluationDateSerial, couponRate, couponFrequencyMonth, couponDcb, accuralDcb, businessDayConvention, periodEndDateConvention, dcCurveId, /* crsCurveId, */ tradeInfo, dayCounters, dayConventions);
...
void inputCouponCashFlow(TradeInformation& tradeInfo, vector<CouponCashflow>& cashflows) {
    ...
    // 지급일 계산 
    // TODO : frequency 관련 인터페이스 논의 필요(현재는 M월 단위로 입력)
    while (startDate < tradeInfo.maturityDate) {
        ...
        cashflow.coupon = tradeInfo.notional * tradeInfo.couponRate * tradeInfo.couponDcb.yearFraction(startDate, endDate);
        // 평가일자 이전의 지급된 현금흐름은 계산하지 않음
        cashflow.yearFrac = (paymentDate < tradeInfo.revaluationDate) ? 0.0 : tradeInfo.accuralDcb.yearFraction(tradeInfo.revaluationDate, paymentDate); 
...
// 커브 데이터 입력 및 초기화
void inputCurveData(const char* curveId, int curveDataSize, const double* yearFrac, const double* marketData, vector<Curve>& curves) {
    
    curves.reserve(curves.capacity() + curveDataSize); // capacity 확보

    for (unsigned int i = 0; i < curveDataSize; ++i) {
...
Real processNetPV(TradeInformation& tradeInfo, vector<CouponCashflow>& cashflows, vector<Curve>& curves) {
    
    // 1. 부트스트래핑으로 discount factor, zero rate 산출
    bootstrapZeroCurveContinuous(curves);
    // 2. Coupon Cashflow PV 산출
...
void inputGirrDelta(const char* curveId, const double maxYearFrac, vector<Girr>& girrs, double* resultGIRRDelta) {
    ...
    // Basis Curve 추가 (USD), Bond 거래 간 단일통화이므로 비활성화
    //  auto processBasisCurve = [&](const char* curveId, const char* currency) {
    //     if (strcmp(currency, "USD") != 0) {
    //         ++resultGIRRDelta[0]; // 유효한 GIRR 커브 수를 카운트 (리턴용)
    //         
    //         Girr basis{};
    //         snprintf(basis.curveId, sizeof(basis.curveId), "%s-Basis", curveId); // 커브 
...
// Net Value 계산
Real calNetPV(vector<CouponCashflow>& cashflows, Real exchangeRate) {
