...
int main() {
    ...
    const int businessDayConventionCode = 1;        // 영업일 조정 Convention (1 = Modified Following) 
    ...
    const long girrDcb = 1;      // GIRR DayCounter (예: 1 = Actual/365)
    const long girrInterpolatorCode = 1;    // 보간법 (예: 1 = Linear)
    const long girrCompoundingCode = 1;     // 이자 계산 방식 (예: 1 = Continuous)
	const long girrFrequencyCode = 1;       // Annual (예: 1 = Annual)
    ...
    const int spreadOverYieldDcb = 1;    // Credit Spread DayCounter (예: 1 = Actual/365)
	...
	const int logYn = 1;			            // 로그 파일 생성 여부 (0: No, 1: Yes)
